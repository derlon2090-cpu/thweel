...
function publicUser(email: string, fullName: string) {
  return {
    id: `session_${email.replace(/[^a-z0-9]/gi, "_").slice(0, 40) || "user"}`,
    email,
    fullName,
...
export async function POST(request: Request) {
  try {
    ...
    return Response.json({ user: publicUser(email, fullName), source: "safe-session" }, { status: 201, headers: { "Set-Cookie": sessionCookie() } });
  } catch {
    return Response.json({ error: "VALIDATION_ERROR", message: REGISTER_ERROR }, { status: 400 });
  }
}
